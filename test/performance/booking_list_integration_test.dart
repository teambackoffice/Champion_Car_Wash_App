...
void main() {
  ...
  group('Booking List Performance Integration Tests', () {
    ...
    testWidgets(
      'Booking list scrolling performance with 100+ items at 60fps',
      (WidgetTester tester) async {
        ...
        await binding.traceAction(() async {
          ...
          final scrollMetrics = await _measureScrollPerformance(tester, listViewFinder);
          
          // Validate performance requirements
          expect(scrollMetrics['averageFrameRate'], greaterThanOrEqualTo(60.0),
              reason: 'Frame rate should be >= 60fps during scrolling');
          expect(scrollMetrics['frameSkips'], lessThan(5),
              reason: 'Should have minimal frame skips during scrolling');
          expect(scrollMetrics['smoothScrollPercentage'], greaterThanOrEqualTo(95.0),
              reason: 'Should maintain smooth scrolling 95% of the time');

          print('Booking List Scrolling Performance Results:');
          print('  Average Frame Rate: ${scrollMetrics['averageFrameRate'].toStringAsFixed(1)} fps');
          print('  Frame Skips: ${scrollMetrics['frameSkips']}');
          print('  Smooth Scroll Percentage: ${scrollMetrics['smoothScrollPercentage'].toStringAsFixed(1)}%');
        }, reportKey: 'booking_list_scrolling_performance');
      },
    );

    testWidgets(
      'Search functionality performance with debouncing',
      (WidgetTester tester) async {
        final binding = IntegrationTestWidgetsFlutterBinding.ensureInitialized();
        ...
        await binding.traceAction(() async {
          ...
          for (final query in searchQueries) {
            ...
            expect(searchMetrics['responseTime'], lessThanOrEqualTo(500.0),
                reason: 'Search response time should be <= 500ms including debouncing');
          }
          ...
          expect(averageResponseTime, lessThanOrEqualTo(400.0),
          ...
          for (final entry in searchResults.entries) {
            print('  "${entry.key}": ${entry.value['responseTime'].toStringAsFixed(1)} ms (${entry.value['resultCount']} results)');
          }
    ...
    testWidgets(
      'Memory stability during extended scrolling and searching',
      (WidgetTester tester) async {
        final binding = IntegrationTestWidgetsFlutterBinding.ensureInitialized();
        ...
        await binding.traceAction(() async {
          ...
          expect(memoryIncrease, lessThan(20.0),
              reason: 'Memory usage should not increase by more than 20MB during extended usage');
    ...
    testWidgets(
      'Complete booking list performance validation',
      (WidgetTester tester) async {
        ...
        expect(searchPerformance['averageResponseTime'], lessThanOrEqualTo(300.0),
        ...
        print('  Search Response Time: ${searchPerformance['averageResponseTime'].toStringAsFixed(1)} ms');
...
List<ServiceData> _generateTestBookingData(int count) {
  ...
  for (int i = 0; i < count; i++) {
    bookings.add(ServiceData(
      ...
      purchaseDate: DateTime.now().subtract(Duration(days: random.nextInt(30))).toIso8601String(),
...
Future<Map<String, dynamic>> _measureScrollPerformance(
  ...
  while (DateTime.now().isBefore(scrollEndTime)) {
    ...
    final frameTime = frameEnd.difference(frameStart).inMicroseconds / 1000.0; // ms
    
    frameMetrics.add(frameTime);
    frameCount++;
    
    if (frameTime > 16.67) { // Frame took longer than 60fps target
      frameSkips++;
    }
  ...
  final averageFrameTime = frameMetrics.reduce((a, b) => a + b) / frameMetrics.length;
...
Future<Map<String, dynamic>> _measureSearchPerformance(
  ...
  final expectedResults = testData.where((booking) =>
    booking.customerName.toLowerCase().contains(query.toLowerCase()) ||
    booking.registrationNumber.toLowerCase().contains(query.toLowerCase())
  ).length;
...
String _getRandomMake(Random random) {
  final makes = ['Toyota', 'Honda', 'BMW', 'Mercedes', 'Audi', 'Ford', 'Chevrolet', 'Nissan'];
  return makes[random.nextInt(makes.length)];
}

String _getRandomModel(Random random) {
  final models = ['Camry', 'Civic', 'X5', 'C-Class', 'A4', 'F-150', 'Silverado', 'Altima'];
  return models[random.nextInt(models.length)];
}

String _getRandomCarType(Random random) {
  final carTypes = ['Sedan', 'SUV', 'Truck', 'Coupe', 'Hatchback', 'Convertible'];
