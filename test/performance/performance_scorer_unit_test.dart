...
void main() {
  group('PerformanceScorer Tests', () {
    ...
    group('Basic Performance Scoring', () {
      ...
      test('should calculate poor performance score', () {
        ...
        expect(
          score.grade,
          anyOf([PerformanceGrade.poor, PerformanceGrade.critical]),
        );
      ...
      test('should use different scoring profiles', () {
        ...
        final championScore = scorer.calculateScore(metrics);
        ...
        expect(
          performanceScore.profile,
          equals(ScoringProfile.performanceFocused),
        );
      });
    });
    ...
    group('Component Scoring', () {
      test('should score startup time correctly', () {
        final excellentStartup = _createTestMetrics(
          startupTime: 800,
        ); // < 1000ms
    ...
    group('Champion Car Wash Workflow Scoring', () {
      ...
      test('should identify critical workflows', () {
        ...
        expect(
          workflowScore.criticalWorkflows,
          contains('Technician Workflow'),
        );
      });
    });
    ...
    group('Device-Specific Scoring', () {
      ...
      test('should have different expectations for device categories', () {
        ...
        final highEndScore = scorer.calculateDeviceScore(
          metrics,
          DeviceCategory.highEnd,
        );
        final midRangeScore = scorer.calculateDeviceScore(
          metrics,
          DeviceCategory.midRange,
        );
        final lowEndScore = scorer.calculateDeviceScore(
          metrics,
          DeviceCategory.lowEnd,
        );
  ...
  group('Enum Tests', () {
    test('ScoringProfile should have correct descriptions', () {
      expect(
        ScoringProfile.championCarWash.description,
        equals('Champion Car Wash optimized scoring'),
      );
      expect(
        ScoringProfile.balanced.description,
        equals('Balanced performance scoring'),
      );
      expect(
        ScoringProfile.performanceFocused.description,
        equals('Performance-focused scoring'),
      );
    });

    test('DeviceCategory should have correct descriptions', () {
      expect(DeviceCategory.highEnd.description, contains('High-end device'));
      expect(DeviceCategory.midRange.description, contains('Mid-range device'));
      expect(DeviceCategory.lowEnd.description, contains('Low-end device'));
    });
    ...
    test('PerformanceGrade should have correct descriptions', () {
      ...
      expect(PerformanceGrade.good.description, equals('Good Performance'));
      expect(PerformanceGrade.fair.description, equals('Fair Performance'));
      expect(PerformanceGrade.poor.description, equals('Poor Performance'));
      expect(
        PerformanceGrade.critical.description,
