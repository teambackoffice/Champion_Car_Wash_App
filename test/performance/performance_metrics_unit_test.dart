...
void main() {
  group('PerformanceMetrics Unit Tests', () {
    group('Constructor and Factory Methods', () {
      ...
      test('should create PerformanceMetrics.now with current timestamp', () {
        ...
        expect(
          metrics.timestamp.isAfter(beforeCreation) ||
              metrics.timestamp.isAtSameMomentAs(beforeCreation),
          isTrue,
        );
        expect(
          metrics.timestamp.isBefore(afterCreation) ||
              metrics.timestamp.isAtSameMomentAs(afterCreation),
          isTrue,
        );
    ...
    group('Health Status Calculation', () {
      test('should return excellent for all requirements met', () {
        ...
        expect(
          excellentMetrics.healthStatus,
          equals(PerformanceHealth.excellent),
        );
      });
    ...
    group('JSON Serialization', () {
      ...
      test('should deserialize from JSON correctly', () {
        ...
        expect(
          ...
          equals({'processor': 'Snapdragon 888', 'ram': 8}),
        );
      });
  ...
  group('PerformanceHealth Enum', () {
    test('should have correct descriptions', () {
      expect(
        PerformanceHealth.excellent.description,
        equals('Excellent - All performance requirements met'),
      );
      expect(
        PerformanceHealth.good.description,
        equals('Good - Most performance requirements met'),
      );
      expect(
        PerformanceHealth.fair.description,
        equals('Fair - Some performance issues detected'),
      );
      expect(
        PerformanceHealth.poor.description,
