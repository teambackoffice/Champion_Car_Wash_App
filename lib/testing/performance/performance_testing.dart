...
class PerformanceTestRunner {
  ...
  static PerformanceTestRunner get instance =>
      _instance ??= PerformanceTestRunner._();
  ...
  Future<void> initialize() async {
    ...
    PerformanceTestHelpers.logPerformanceInfo(
  ...
  Future<void> cleanup() async {
    ...
    PerformanceTestHelpers.logPerformanceInfo(
      'Performance testing framework cleaned up',
    );
  }
  ...
  Future<List<TestResults>> runAllTests() async {
    ...
    try {
      // Run startup tests
      PerformanceTestHelpers.logPerformanceInfo(
        'Running startup performance tests...',
      );
      final startupResult = await PerformanceTestSuite.runStartupTests();
      ...
      PerformanceTestHelpers.logPerformanceInfo(
        'Overall performance score: ${overallScore.score}/10 (${overallScore.grade})',
      );
    } catch (e) {
      PerformanceTestHelpers.logPerformanceInfo(
  ...
  Future<List<TestResults>> runWorkflowTests() async {
    ...
    PerformanceTestHelpers.logPerformanceInfo(
      'Workflow-specific tests will be implemented in tasks 2.1-2.4',
    );
...
class PerformanceTest {
  /// Quick startup test
  static Future<TestResults> startup() =>
      PerformanceTestSuite.runStartupTests();

  /// Quick memory test
  static Future<TestResults> memory() => PerformanceTestSuite.runMemoryTests();

  /// Quick UI test
  static Future<TestResults> ui() =>
      PerformanceTestSuite.runUIPerformanceTests();

  /// Quick API test
  static Future<TestResults> api() => PerformanceTestSuite.runAPITests();

  /// Run all tests
  static Future<List<TestResults>> all() =>
      PerformanceTestRunner.instance.runAllTests();

  /// Get current metrics
  static Future<PerformanceMetrics> metrics() =>
      PerformanceTestRunner.instance.getCurrentMetrics();

  /// Generate report
  static Future<String> report() =>
      PerformanceTestRunner.instance.generateReport();
}
