...
class StartupPerformanceTest extends BasePerformanceTest {
  ...
  String get description =>
      'Validates app startup time under 2 seconds with 35+ StatefulWidgets';
  ...
  bool _validateStartupPerformance(Map<String, dynamic> metrics) {
    ...
    final paintRequirement =
        firstPaint < (targetStartupTime * 0.6); // 60% of total time

    if (kDebugMode) {
      print('Startup Performance Validation:');
      print(
        '  Time requirement (< ${targetStartupTime}ms): $timeRequirement (${totalTime}ms)',
      );
      print(
        '  Widget requirement (>= $minStatefulWidgets): $widgetRequirement ($widgetCount widgets)',
      );
      print(
        '  Paint requirement (< ${targetStartupTime * 0.6}ms): $paintRequirement (${firstPaint}ms)',
      );
    }
    ...
    return {
      ...
      'validation': {
        ...
        'widgetCheck':
            ...
            minStatefulWidgets,
        'paintCheck':
            (result.additionalMetrics['firstMeaningfulPaint'] as double) <
            (targetStartupTime * 0.6),
      },
    };
  }
}
