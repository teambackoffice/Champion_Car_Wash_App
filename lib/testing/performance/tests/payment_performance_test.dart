...
class PaymentPerformanceTest extends BasePerformanceTest {
  ...
  static const double maxStripeInitTime = 1000.0; // 1 second for Stripe initialization
  static const double maxNfcResponseTime = 2000.0; // 2 seconds for NFC response
  static const double maxPaymentProcessingTime = 5000.0; // 5 seconds for payment processing
  static const double maxPaymentHistoryLoadTime = 1500.0; // 1.5 seconds for history loading
  
  @override
  String get testName => _testName;
  
  @override
  String get description => 'Validates Stripe NFC UI responsiveness and payment processing performance';
  ...
  Future<TestResults> executeTest() async {
    ...
    try {
      ...
      final overallProcessingTime = paymentMetrics['overallProcessingTime'] as double;
  ...
  Future<Map<String, dynamic>> _measurePaymentPerformance() async {
    ...
    final overallMetrics = _calculateOverallPaymentMetrics(
      stripeMetrics, nfcMetrics, processingMetrics, historyMetrics, uiMetrics
    );
  ...
  Future<Map<String, dynamic>> _measureNfcResponsiveness() async {
    ...
    final averageUiUpdateTime = uiUpdateTimes.reduce((a, b) => a + b) / uiUpdateTimes.length;
  ...
  Map<String, dynamic> _calculateOverallPaymentMetrics(
    ...
    if (processingTime > maxPaymentProcessingTime) {
      score -= (processingTime - maxPaymentProcessingTime) / maxPaymentProcessingTime * 3.0;
    }
    
    // Deduct points for slow history loading
    if (historyTime > maxPaymentHistoryLoadTime) {
      score -= (historyTime - maxPaymentHistoryLoadTime) / maxPaymentHistoryLoadTime * 1.0;
    }
  ...
  bool _validatePaymentPerformance(Map<String, dynamic> metrics) {
    final stripeTime = metrics['stripeInitialization']['totalInitTime'] as double;
    final nfcTime = metrics['nfcResponsiveness']['totalNfcTime'] as double;
    final processingTime = metrics['paymentProcessing']['totalProcessingTime'] as double;
    final historyTime = metrics['paymentHistory']['totalHistoryLoadTime'] as double;
    ...
    if (kDebugMode) {
      ...
      print('  Stripe init requirement (<= ${maxStripeInitTime}ms): $stripeRequirement (${stripeTime.toStringAsFixed(1)}ms)');
      print('  NFC response requirement (<= ${maxNfcResponseTime}ms): $nfcRequirement (${nfcTime.toStringAsFixed(1)}ms)');
      print('  Processing requirement (<= ${maxPaymentProcessingTime}ms): $processingRequirement (${processingTime.toStringAsFixed(1)}ms)');
      print('  History loading requirement (<= ${maxPaymentHistoryLoadTime}ms): $historyRequirement (${historyTime.toStringAsFixed(1)}ms)');
      print('  UI responsiveness requirement: $uiRequirement');
    }
    
    return stripeRequirement && nfcRequirement && processingRequirement && 
           historyRequirement && uiRequirement;
  }
  ...
  static Future<Map<String, dynamic>> getPaymentReport() async {
    ...
    return {
      ...
      'validation': {
        'stripeCheck': (result.additionalMetrics['stripeInitialization']['totalInitTime'] as double) <= maxStripeInitTime,
        'nfcCheck': (result.additionalMetrics['nfcResponsiveness']['totalNfcTime'] as double) <= maxNfcResponseTime,
        'processingCheck': (result.additionalMetrics['paymentProcessing']['totalProcessingTime'] as double) <= maxPaymentProcessingTime,
        'historyCheck': (result.additionalMetrics['paymentHistory']['totalHistoryLoadTime'] as double) <= maxPaymentHistoryLoadTime,
        'uiCheck': result.additionalMetrics['uiResponsiveness']['uiResponsive'] as bool,
      },
    };
  }
}