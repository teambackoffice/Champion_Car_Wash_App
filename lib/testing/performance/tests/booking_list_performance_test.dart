...
class BookingListPerformanceTest extends BasePerformanceTest {
  ...
  static const double maxSearchResponseTime =
      300.0; // 300ms for search debouncing
  ...
  Future<Map<String, dynamic>> _measureBookingListPerformance() async {
    ...
    final scrollMetrics = await _measureScrollingPerformance(
      bookingItems.length,
    );
  ...
  Future<List<Map<String, dynamic>>> _generateBookingTestData() async {
    ...
    if (kDebugMode) {
      print(
        'Generated ${bookings.length} booking items for performance testing',
      );
    }
  ...
  Future<Map<String, dynamic>> _measureScrollingPerformance(
    int itemCount,
  ) async {
    ...
    final overallSmoothness =
        ((totalFrames - totalFrameSkips) / totalFrames) * 100;
    ...
    return {
      ...
      'performanceGrade': _calculateScrollPerformanceGrade(
        averageFrameRate,
        totalFrameSkips,
        totalFrames,
      ),
  ...
  Future<Map<String, dynamic>> _measureScrollScenario(
    ...
    for (int frame = 0; frame < frameCount; frame++) {
      ...
      final frameComplexity =
          _calculateFrameComplexity(frame, itemCount) * intensity;
      await Future.delayed(
        Duration(microseconds: (frameComplexity * 1000).round()),
      );
  ...
  Future<Map<String, dynamic>> _measureMomentumScrolling(int itemCount) async {
    ...
    while (velocity > minVelocity) {
      ...
      final frameComplexity =
          _calculateFrameComplexity(frameCount, itemCount) * velocityFactor;
      await Future.delayed(
        Duration(microseconds: (frameComplexity * 1000).round()),
      );
  ...
  String _calculateScrollPerformanceGrade(
    double frameRate,
    int frameSkips,
    int totalFrames,
  ) {
  ...
  Future<Map<String, dynamic>> _testRapidTypingPerformance(
    List<Map<String, dynamic>> bookings,
  ) async {
    ...
    return {
      ...
      'debounceEfficiency':
          actualSearches == 1, // Should only search once due to debouncing
    };
  }
  ...
  double _calculateCacheHitRate(
    ...
    return queries.length > 1
        ? (cacheHits / (queries.length - 1)) * 100.0
        : 0.0;
  }
  ...
  double _calculateFrameComplexity(int frameIndex, int itemCount) {
    ...
    final scrollSpeed =
        (frameIndex % 60) / 60.0; // Simulate varying scroll speed
  ...
  double _calculateSearchAccuracy(
    String query,
  ...
  Map<String, dynamic> _calculateOverallMetrics(
    ...
    if (searchResponseTime > maxSearchResponseTime) {
      score -=
          (searchResponseTime - maxSearchResponseTime) /
          maxSearchResponseTime *
          2.0;
    }
  ...
  bool _validateListPerformance(Map<String, dynamic> metrics) {
    ...
    final searchTime =
        metrics['searchPerformance']['averageResponseTime'] as double;
    ...
    if (kDebugMode) {
      ...
      print(
        '  Frame rate requirement (>= ${targetFrameRate}fps): $frameRateRequirement (${frameRate.toStringAsFixed(1)}fps)',
      );
      print(
        '  Item count requirement (>= $minBookingItems): $itemCountRequirement ($itemCount items)',
      );
      print(
        '  Search requirement (<= ${maxSearchResponseTime}ms): $searchRequirement (${searchTime.toStringAsFixed(1)}ms)',
      );
    }
  ...
  validateChampionCarWashRequirements() async {
    final test = BookingListPerformanceTest();
    final result = await test.execute();

    final metrics = result.additionalMetrics;
    final scrollPerformance =
        metrics['scrollPerformance'] as Map<String, dynamic>;
    ...
    final requirements = {
      'frameRate': {
        ...
        'details':
            'Frame rate during list scrolling with ${metrics['bookingItemCount']} items',
      },
      'itemCount': {
      ...
      'searchResponse': {
        ...
        'passed':
            (searchPerformance['averageResponseTime'] as double) <=
            maxSearchResponseTime,
        'details':
            'Average search response time including 300ms debounce delay',
      },
      'scrollSmoothness': {
        ...
        'passed':
            (scrollPerformance['smoothScrollPercentage'] as double) >= 95.0,
        'details': 'Percentage of frames rendered within 16.67ms target',
      },
      'debounceEfficiency': {
        'requirement':
            'Efficient search debouncing to prevent excessive API calls',
        'target': 1.0, // Should only trigger one search per typing sequence
        'actual':
            (searchPerformance['rapidTypingMetrics']['actualSearches'] as int)
                .toDouble(),
        'passed':
            searchPerformance['rapidTypingMetrics']['debounceEfficiency']
                as bool,
    ...
    final passedRequirements = requirements.values
        .where((req) => req['passed'] as bool)
        .length;
  ...
  static Future<Map<String, dynamic>> getBookingListReport() async {
    ...
    return {
      ...
      'validation': {
        ...
        'itemCountCheck':
            (result.additionalMetrics['bookingItemCount'] as int) >=
            minBookingItems,
        'searchCheck':
            (result.additionalMetrics['searchPerformance']['averageResponseTime']
                as double) <=
            maxSearchResponseTime,
      },
    };
