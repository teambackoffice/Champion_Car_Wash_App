...
class MemoryStabilityTest extends BasePerformanceTest {
  ...
  Future<Map<String, dynamic>> _measureMemoryStability(
    ...
    if (kDebugMode) {
      print(
        'Starting memory stability test for ${testDuration.inMinutes} minutes',
      );
    ...
    while (DateTime.now().isBefore(endTime)) {
      ...
      final leakCheck = await _checkForMemoryLeaks(
        baselineMemory,
        currentMemory,
        cycleCount,
      );
    ...
    return {
      ...
      'disposalIssuesDetected': widgetDisposalChecks
          .where((check) => !check['properDisposal'])
          .length,
      'averageMemoryUsage':
          memoryReadings
              .map((r) => r['memoryUsage'] as double)
              .reduce((a, b) => a + b) /
  ...
  Future<Map<String, dynamic>> _simulateTechnicianWorkflowCycle(
    int cycleNumber,
  ) async {
    final workflowTypes = [
      'car_wash',
  ...
  Future<Map<String, dynamic>> _simulateFullServiceWorkflow() async {
    ...
    return {
      ...
      'duration':
          (carWashMetrics['duration'] as int) +
          (oilChangeMetrics['duration'] as int) +
          800,
  ...
  Future<Map<String, dynamic>> _checkForMemoryLeaks(
    double baseline,
    double current,
    int cycle,
  ) async {
    ...
    if (leakDetected) {
      ...
      if (kDebugMode) {
        print(
          'Memory leak detected at cycle $cycle: ${memoryGrowth.toStringAsFixed(1)}MB growth',
        );
      }
    }
  ...
  bool _validateMemoryStability(Map<String, dynamic> metrics) {
    ...
    final testDuration = Duration(
      milliseconds: (metrics['testDuration'] as double).round(),
    );
    ...
    if (kDebugMode) {
      ...
      print(
        '  Memory usage requirement (<= ${maxMemoryUsage}MB): $memoryRequirement (${maxMemory.toStringAsFixed(1)}MB)',
      );
      print(
        '  Memory leak requirement (0 leaks): $leakRequirement ($leaksDetected leaks)',
      );
      print(
        '  Memory growth requirement (<= ${memoryLeakThreshold}MB): $growthRequirement (${memoryGrowth.toStringAsFixed(1)}MB)',
      );
      print(
        '  Test duration requirement (>= ${defaultTestDuration.inMinutes}min): $durationRequirement (${testDuration.inMinutes}min)',
      );
      print(
        '  Widget disposal requirement (0 issues): $disposalRequirement ($disposalIssues issues)',
      );
    }
  ...
  static Future<Map<String, dynamic>> getMemoryReport({
    Duration? testDuration,
  }) async {
    final test = MemoryStabilityTest();
    ...
    return {
      ...
      'validation': {
        ...
        'leakCheck':
            (result.additionalMetrics['memoryLeaksDetected'] as int) == 0,
        'growthCheck':
            (result.additionalMetrics['memoryGrowth'] as double) <=
            memoryLeakThreshold,
        'disposalCheck':
            (result.additionalMetrics['disposalIssuesDetected'] as int) == 0,
      },
    };
  }
}
