...
class FlutterPerformanceBinding {
  ...
  static FlutterPerformanceBinding get instance => _instance ??= FlutterPerformanceBinding._();
  ...
  Future<double> measureWidgetBuildTime(WidgetTester tester, Widget widget) async {
  ...
  Future<Map<String, dynamic>> measureScrollingPerformance(
    ...
    final scrollingFrameRates = frameRatesAfter.skip(frameRatesBefore.length).toList();
    
    return {
      'averageFrameRate': scrollingFrameRates.isNotEmpty 
          ? scrollingFrameRates.reduce((a, b) => a + b) / scrollingFrameRates.length
          : 0.0,
      'minFrameRate': scrollingFrameRates.isNotEmpty ? scrollingFrameRates.reduce((a, b) => a < b ? a : b) : 0.0,
  ...
  Future<Map<String, dynamic>> measureAnimationPerformance(
    ...
    final animationFrameTimings = _frameTimings.skip(frameTimingsBefore.length).toList();
  ...
  Future<List<String>> detectStatefulWidgetLeaks(WidgetTester tester) async {
    ...
    void visitor(Element element) {
      if (element.widget is StatefulWidget) {
        ...
        if (stateString.contains('Controller') && !stateString.contains('disposed')) {
  ...
  int _countFrameSkips() {
    ...
    return _frameTimings
        .where((timing) => timing.totalSpan.inMicroseconds > targetFrameTime * 1.5)
        .length;
  }
  ...
  Map<String, dynamic> _analyzeFrameTimings(List<FrameTiming> timings) {
    ...
    final frameDrops = frameTimes.where((time) => time > 16667 * 1.5).length; // > 1.5x target
