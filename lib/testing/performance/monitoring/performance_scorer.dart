...
class PerformanceScorer {
  ...
  PerformanceScore calculateScore(PerformanceMetrics metrics, {
    ...
    final startupScore = _calculateStartupScore(metrics.startupTimeMs, thresholds);
    final frameRateScore = _calculateFrameRateScore(metrics.frameRate, thresholds);
    final memoryScore = _calculateMemoryScore(metrics.memoryUsageMB, thresholds);
    final apiScore = _calculateApiScore(metrics.apiResponseTimeMs, thresholds);
    final stabilityScore = _calculateStabilityScore(metrics.frameSkips, thresholds);
    final batteryScore = _calculateBatteryScore(metrics.batteryDrainPerHour, thresholds);
  ...
  ChampionCarWashScore calculateWorkflowScore(PerformanceMetrics metrics) {
    ...
    final overallWorkflowScore = (
      ...
      customerScore * 0.15      // Customer interactions are important but less frequent
    ).clamp(0.0, 10.0);
  ...
  double _calculateStartupScore(double startupTimeMs, ScoringThresholds thresholds) {
  ...
  double _calculateFrameRateScore(double frameRate, ScoringThresholds thresholds) {
  ...
  double _calculateMemoryScore(double memoryUsageMB, ScoringThresholds thresholds) {
  ...
  double _calculateApiScore(double apiResponseTimeMs, ScoringThresholds thresholds) {
  ...
  double _calculateStabilityScore(int frameSkips, ScoringThresholds thresholds) {
  ...
  double _calculateBatteryScore(double batteryDrainPerHour, ScoringThresholds thresholds) {
  ...
  double _calculateTechnicianWorkflowScore(PerformanceMetrics metrics) {
    ...
    final frameRateScore = _calculateFrameRateScore(metrics.frameRate, 
        ScoringThresholds.championCarWash());
    ...
    return (startupScore * startupWeight + 
            ...
            apiScore * apiWeight).clamp(0.0, 10.0);
  }
  ...
  double _calculateSupervisorWorkflowScore(PerformanceMetrics metrics) {
    ...
    final apiScore = _calculateApiScore(metrics.apiResponseTimeMs, 
        ScoringThresholds.championCarWash());
    final stabilityScore = _calculateStabilityScore(metrics.frameSkips, 
        ScoringThresholds.championCarWash());
    
    return (memoryScore * memoryWeight + 
            apiScore * apiWeight + 
            stabilityScore * stabilityWeight).clamp(0.0, 10.0);
  }
  ...
  double _calculatePaymentWorkflowScore(PerformanceMetrics metrics) {
    ...
    return (apiScore * apiWeight + 
            ...
            memoryScore * memoryWeight).clamp(0.0, 10.0);
  }
  ...
  double _calculateCustomerWorkflowScore(PerformanceMetrics metrics) {
    ...
    final frameRateScore = _calculateFrameRateScore(metrics.frameRate, 
        ScoringThresholds.championCarWash());
    final batteryScore = _calculateBatteryScore(metrics.batteryDrainPerHour, 
        ScoringThresholds.championCarWash());
    
    return (startupScore * startupWeight + 
            frameRateScore * frameRateWeight + 
            batteryScore * batteryWeight).clamp(0.0, 10.0);
  }
  ...
  Map<String, double> _calculateAllComponentScores(
    ...
    return {
      ...
      'battery': _calculateBatteryScore(metrics.batteryDrainPerHour, thresholds),
    };
  }
...
class PerformanceScore {
  ...
  String generateReport() {
    ...
    buffer.writeln('Overall Score: ${overall.toStringAsFixed(1)}/10 (${grade.description})');
    buffer.writeln();
