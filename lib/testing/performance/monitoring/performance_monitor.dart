...
class PerformanceMonitor {
  ...
  void startMonitoring({Duration interval = const Duration(seconds: 1)}) {
    ...
    developer.log(
      'PerformanceMonitor: Starting monitoring with ${interval.inSeconds}s interval',
    );
    ...
    if (metrics.startupTimeMs > _thresholds.maxStartupTimeMs) {
      _triggerAlert(
        AlertEvent(
          ...
          message:
              'Slow startup detected: ${metrics.startupTimeMs.toStringAsFixed(0)}ms',
          timestamp: DateTime.now(),
        ),
      );
    }
    ...
    if (metrics.memoryUsageMB > _thresholds.maxMemoryUsageMB) {
      _triggerAlert(
        AlertEvent(
          ...
          message:
              'High memory usage: ${metrics.memoryUsageMB.toStringAsFixed(1)}MB',
          timestamp: DateTime.now(),
        ),
      );
    }
    ...
    if (metrics.batteryDrainPerHour > _thresholds.maxBatteryDrainPerHour) {
      _triggerAlert(
        AlertEvent(
          ...
          message:
              'High battery drain: ${metrics.batteryDrainPerHour.toStringAsFixed(1)}%/hour',
          timestamp: DateTime.now(),
        ),
      );
    }
  ...
  void _triggerAlert(AlertEvent alert) {
    ...
    developer.log(
      'PerformanceMonitor Alert [${alert.level}]: ${alert.message}',
    );
  }
}
